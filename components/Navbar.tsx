...
export default function Navbar() {
  ...
  return (
    <header
      ...
      } ${scrolled ? 'bg-forest/90 backdrop-blur-sm py-4' : 'bg-transparent py-7'}`}
    >
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 md:px-10">
        ...
        <ul className="hidden gap-9 md:flex">
          {site.nav.map((item) => (
            <li key={item.href}>
                ...
                href={item.href}
                className="text-[13px] font-medium tracking-wide text-ivory/80 transition-colors duration-300 hover:text-gold"
              >
                {item.label}
              </a>
            </li>
        ...
        >
          <span
            className={`h-px bg-ivory transition-all duration-300 ${
